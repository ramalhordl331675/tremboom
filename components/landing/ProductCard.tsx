...
export function ProductCard({ product: p }: { product: LandingProductItem }) {
  ...
  return (
    <li
      key={p.id}
      className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-[0_12px_32px_-12px_rgb(234_71_12/0.45)]"
    >
      <ProductThumb product={p} />
      <div className="flex min-w-0 flex-1 flex-col gap-1.5 p-3 sm:p-4">
        <p className="line-clamp-2 min-h-10 text-[13px] font-semibold leading-snug break-words text-[#3d2c25]">
          {p.name}
        </p>
        {rating !== null ? (
        ...
        <p className="text-lg font-black break-words text-[#EA470C] sm:text-xl">{brl(price)}</p>
        ...
        <a
          ...
          className="mt-auto block rounded-xl bg-gradient-to-r from-[#F96116] to-[#E63A1E] px-3 py-2.5 pt-2.5 text-center text-xs font-black uppercase leading-snug tracking-wide break-words text-white transition hover:brightness-110 min-[400px]:px-4 min-[400px]:text-[13px]"
